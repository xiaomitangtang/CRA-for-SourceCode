...
function testStream() {
  ...
  可读流是对提供数据的来源的一种抽象。
      ...
      'readable' 事件 当有数据可从流中读取时，就会触发 'readable' 事件。    在此事件中调用read读取数据
                    'readable' 事件表明流有新的动态：要么有新的数据，要么到达流的尽头。 对于前者，stream.read() 会返回可用的数据。 对于后者，stream.read() 会返回 null。      
         同时使用 'readable' 事件和 'data' 事件，则 'readable' 事件会优先控制流
      'resume' 事件
    ...
    双工流与转换流
      ...
      stream.Readable.from(iterable, [options])
          调用 Readable.from(string) 或 Readable.from(buffer) 将不会迭代字符串或 buffer 以匹配其他流的语义
  */
  function servers() {
  ...
  function ReadableFrom() {
    let source = [1, 2, 43, 5, 6, 7]
    source = '12345'
    stream.Readable.from(source).on('data', e => {
      console.log('fromStream:', e)
    })
  }
  ReadableFrom()

}
function test_string_decoder() {
  const { StringDecoder } = require('string_decoder');


  /*
  能保护已编码的多字节 UTF-8 和 UTF-16 字符的方式将 Buffer 对象解码为字符串。
    stringDecoder.end([buffer])
        将 Buffer 实例写入 StringDecoder 实例时，会使用一个内部 buffer 来确保解码的字符串不包含任何不完整的多字节字符。 不完整的字符会被保存在该 buffer 中，直到下次调用 stringDecoder.write() 或调用 stringDecoder.end() 。
